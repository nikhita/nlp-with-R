...
toknized <-  annotate(aSentence, list(sent_token_annotator, word_token_annotator)) 
#toknized


# Get NE Recognizer
per_ner <- openNLP::Maxent_Entity_Annotator(kind = "person")
#org_ner <- Maxent_Entity_Annotator(kind = "organization")
#location_ner <- Maxent_Entity_Annotator(kind = "location")

# Annotate/perform NER
nameAnnotated <- annotate(aSentence, per_ner, toknized)
#nameAnnotated

# Print the NEs
namedEntities <- subset(nameAnnotated, type =="entity")
aSentence[namedEntities]
